...
function vevent(ev: TrackEvent, opts: ICSOptions): string {
  ...
  const desc = [ev.note, ev.url ? "Link: " + ev.url : "", "Tracked in Sooncast"].filter(Boolean).join("\\n");
...
export function buildICS(events: TrackEvent[], opts: ICSOptions = {}): string {
  const head = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
    "PRODID:-//Sooncast//Universal Tracker//EN",
    "CALSCALE:GREGORIAN",
    "METHOD:PUBLISH",
    `X-WR-CALNAME:${icsEscape(opts.calName || "Sooncast")}`,
  ];
